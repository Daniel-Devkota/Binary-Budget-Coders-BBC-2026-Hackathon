...
import {
  ...
  Home, Sparkles, HelpCircle, MapPin,
} from 'lucide-react'
      ...
      <header className="sticky top-0 z-40 border-b-2 border-line-strong bg-paper/90 backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 h-16 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Link
              to="/profile"
              className="hidden sm:flex items-center gap-2 pl-1 pr-2 h-10 rounded-[12px] hover:bg-paper-deep"
            >
        ...
        {open && (
          <nav className="lg:hidden border-t-2 border-line px-4 py-3 grid gap-1 bg-paper" aria-label="Mobile">
            {[...nav, { to: '/profile', label: 'Your profile', icon: User }, { to: '/map', label: 'Map', icon: MapPin }].map(
              ({ to, label, icon: Icon }) => (
                <NavLink
                  key={to}
                  to={to}
                  ...
                  className={({ isActive }) =>
                    cn(
                      ...
                      isActive ? 'bg-indigo-500 text-white' : 'text-ink-soft hover:bg-paper-deep',
                    )
                  }
                >
                  <Icon className="size-4" aria-hidden />
